...
async def get_my_resources(db: Session = Depends(get_db)):
    ...
    일반 유저: 본인 소유 자원만 조회
    """
    current_user = "admin"  # TODO: 실제 환경에서는 인증 토큰에서 추출

    # 1. DB 조회 (is_used 대신 status 필드가 있는 WorkloadPool 클래스 사용)
    if current_user == "admin":
        my_vms = db.query(WorkloadPool).all()
    else:
        my_vms = db.query(WorkloadPool).filter(
            WorkloadPool.occupy_user == current_user
        ).all()
...
async def update_settings(req: SettingsUpdateRequest, db: Session = Depends(get_db)):
    ...
    if req.admin_password != s.admin_password:
        raise HTTPException(status_code=403, detail="비밀번호 오류")
